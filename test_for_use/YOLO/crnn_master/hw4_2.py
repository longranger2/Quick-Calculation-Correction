...
import argparse
from crnn_master.crnn import CRNN
...
def detect_(crnn, source, device, converter):
    ...
    if os.path.isfile(source): #判断某一对象(需提供绝对路径)是否为文件
        ...
        for s in source_list:
            ...
            equations.append(detect(crnn, os.path.join(source, s), device, converter))
    return equations
...
def parse_opt():
    ...
    # 构建到YOLO目录的相对路径
    yolo_path = os.path.join(current_path, '..', 'yolo3/tmp_img/')

    # 规范化路径，确保路径格式正确
    yolo_path = os.path.normpath(yolo_path)
    
    # parser.add_argument('--weights', type=str, default='../crnn_master/weights/CPU.pt', help='权重的路径')
    parser.add_argument('--weights', type=str, default=os.path.join(current_path, 'weights', 'CPU.pt'), help='权重的路径')
    # parser.add_argument('--source', type=str, default='../YOLO/yolo3/tmp_img/', help='要用来推理图片的路径，可以是一张图片，也可以是一个目录')
    parser.add_argument('--source', type=str, default=yolo_path, help='要用来推理图片的路径，可以是一张图片，也可以是一个目录')
    parser.add_argument('--batch_size', type=int, default=32, help='批次大小')
    # parser.add_argument('--chinese', type=str, default='../crnn_master/data/formula.txt', help='字符集保存路径')
    parser.add_argument('--chinese', type=str, default=os.path.join(current_path, 'utils', 'data', 'formula.txt'), help='字符集保存路径')
